...
async def require_search_quota(current_user: UserInfo = Depends(get_current_active_user)) -> UserInfo:
    ...
    if not has_quota:
        ...
        raise QuotaError.quota_exceeded(
            quota_type="搜索", used=quota_status.get('used_today', 0), limit=quota_status.get('daily_limit', 0)
        )
    return current_user
...
async def require_ai_search_quota(current_user: UserInfo = Depends(get_current_active_user)) -> UserInfo:
    ...
    if not has_quota:
        ...
        raise QuotaError.quota_exceeded(
            quota_type="AI搜索", used=quota_status.get('used_today', 0), limit=quota_status.get('daily_limit', 0)
        )
    return current_user
