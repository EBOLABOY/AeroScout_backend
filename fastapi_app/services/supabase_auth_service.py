...
class SupabaseAuthService:
    ...
    def __init__(self):
        """初始化 Supabase 认证服务"""
        # 使用 anon key 进行认证操作
        self.client: Client | None = get_supabase_client(use_service_key=False)
        self.admin_client: Client | None = get_supabase_client(use_service_key=True)
    ...
    async def sign_up_with_email(
        self, email: str, password: str, username: str, user_metadata: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            response = self.client.auth.sign_up(
                {"email": email, "password": password, "options": {"data": metadata, "redirect_to": self.redirect_url}}
            )
    ...
    async def verify_email(self, token: str, email: str) -> dict[str, Any]:
        ...
        try:
            ...
            response = self.client.auth.verify_otp({"token": token, "type": "signup", "email": email})

            if response.user and response.session:
                logger.info(f"邮箱验证成功: {email}")
                return {"success": True, "message": "邮箱验证成功", "user": response.user, "session": response.session}
            else:
    ...
    async def resend_confirmation_email(self, email: str) -> dict[str, Any]:
        ...
        try:
            ...
            self.client.auth.resend({"type": "signup", "email": email, "options": {"redirect_to": self.redirect_url}})
    ...
    async def send_password_reset_email(self, email: str) -> dict[str, Any]:
        ...
        try:
            ...
            self.client.auth.reset_password_email(email, {"redirect_to": f"{self.site_url}/auth/reset-password"})
    ...
    async def update_password(self, access_token: str, new_password: str) -> dict[str, Any]:
        ...
        try:
            ...
            if response.user:
                ...
                return {"success": True, "message": "密码更新成功", "user": response.user}
            else:
    ...
    async def update_email(self, access_token: str, new_email: str) -> dict[str, Any]:
        ...
        try:
            ...
            response = self.client.auth.update_user({"email": new_email})
    ...
    async def send_magic_link(self, email: str) -> dict[str, Any]:
        ...
        try:
            ...
            self.client.auth.sign_in_with_otp(
                {"email": email, "options": {"should_create_user": False, "redirect_to": self.redirect_url}}
            )
    ...
    async def verify_magic_link(self, token: str, email: str) -> dict[str, Any]:
        ...
        try:
            ...
            response = self.client.auth.verify_otp({"token": token, "type": "email", "email": email})

            if response.user and response.session:
                logger.info(f"魔法链接验证成功: {email}")
                return {"success": True, "message": "登录成功", "user": response.user, "session": response.session}
            else:
    ...
    async def sign_in_with_google(self, redirect_to: str | None = None) -> dict[str, Any]:
        """
        使用Google OAuth登录
        ...
        try:
            ...
            # 发起Google OAuth登录
            response = self.client.auth.sign_in_with_oauth({"provider": "google", "options": options})

            if response.url:
                logger.info("Google OAuth登录URL生成成功")
                return {"success": True, "auth_url": response.url, "message": "请跳转到授权URL完成Google登录"}
            else:
    ...
    async def handle_oauth_callback(self, code: str, state: str | None = None) -> dict[str, Any]:
        """
        处理OAuth回调
...
# 全局服务实例
_supabase_auth_service: SupabaseAuthService | None = None
