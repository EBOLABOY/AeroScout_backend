...
class CacheService:
    ...
    async def connect(self):
        ...
        try:
            ...
            self.redis = await aioredis.from_url(
                redis_url, encoding="utf-8", decode_responses=True, max_connections=20, retry_on_timeout=True
            )
    ...
    async def get_monitor_tasks(self, user_id: int) -> dict[str, Any] | None:
        """获取用户监控任务列表缓存"""
        key = f"monitor:tasks:{user_id}"
        return await self.get(key, dict)
    ...
    async def cache_flight_search_results(self, search_params: dict[str, Any], results: list[dict[str, Any]]) -> bool:
        """缓存航班搜索结果（10分钟过期）"""
        # 生成缓存键
        ...
        cache_data = {'search_params': search_params, 'results': results, 'cached_at': datetime.now().isoformat()}

        return await self.set(key, cache_data, expire=600)  # 10分钟

    async def get_flight_search_results(self, search_params: dict[str, Any]) -> list[dict[str, Any]] | None:
        """获取航班搜索结果缓存"""
        params_str = json.dumps(search_params, sort_keys=True, ensure_ascii=False)
        import hashlib
    ...
    async def get_airports_list(self) -> list[dict[str, Any]] | None:
        """获取机场列表缓存"""
        key = "airports:list"
    ...
    async def get_monitor_stats(self) -> dict[str, Any] | None:
        """获取监控统计信息缓存"""
        key = "monitor:stats"
        return await self.get(key, dict)
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            if retrieved_value == test_value:
                ...
                return {'status': 'error', 'message': 'Redis读写测试失败'}

        except Exception as e:
            return {'status': 'error', 'message': f'Redis健康检查失败: {str(e)}'}
