...
class SupabaseUserSyncService:
        ...
        try:
            ...
            if existing_user:
                ...
                return await self._update_existing_user(
                    existing_user, sync_data, table_name
                )
            else:
                # 新用户：创建记录
                return await self._create_new_user(
                    user_id, sync_data, table_name
                )
    ...
    def _build_sync_data(self, supabase_user) -> Dict[str, Any]:
        ...
        if hasattr(supabase_user, 'last_sign_in_at') and supabase_user.last_sign_in_at:
            ...
            sync_data["last_login"] = datetime.now(timezone.utc).isoformat()
        
        # 处理用户名（从metadata或邮箱生成）
        if (supabase_user.user_metadata and 
    ...
    async def batch_sync_auth_users(
        self, 
        auth_users: List[Any],
        table_name: str = "profiles"
    ) -> Dict[str, int]:
        ...
        for auth_user in auth_users:
            try:
                result = await self.sync_auth_user_to_business_table(
                    auth_user, table_name
                )
