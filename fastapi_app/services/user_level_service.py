...
class UserLevelService:
    ...
    async def get_all_user_levels(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            result = db_service.client.table("user_levels").select("*").eq("is_active", True).order("sort_order").execute()
    ...
    async def get_user_level_by_id(self, level_id: int) -> Optional[Dict[str, Any]]:
        """根据ID获取用户等级"""
        try:
            db_service = await get_supabase_service()
    ...
    async def get_user_level_by_name(self, level_name: str) -> Optional[Dict[str, Any]]:
        """根据名称获取用户等级"""
        try:
            db_service = await get_supabase_service()
            result = db_service.client.table("user_levels").select("*").eq("name", level_name).eq("is_active", True).execute()
    ...
    async def get_user_level_permissions(self, level_name: str) -> List[str]:
        """获取用户等级对应的权限列表"""
        from fastapi_app.dependencies.permissions import Role, ROLE_PERMISSIONS
    ...
    async def get_user_level_benefits(self, level_name: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "benefits": benefits.get(level_name, benefits["user"])
        }
