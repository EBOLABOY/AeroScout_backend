...
from ..dependencies.permissions import Permission, check_permission
...
async def check_admin_permission(current_user: UserInfo) -> None:
    ...
    if not check_permission(current_user, Permission.SYSTEM_ADMIN):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="缺少系统管理权限")
    ...
    try:
        ...
        try:
            active_users_result = (
                ...
                .filter('last_login', 'gte', thirty_days_ago)
                .execute()
            )
            ...
            active_users_result = (
                supabase_service.client.table('profiles').select('id', count='exact').eq('is_active', True).execute()
            )
            active_users = active_users_result.count if active_users_result.count is not None else 0
        ...
        try:
            ...
            active_tasks_result = (
                supabase_service.client.table('monitor_tasks')
                .select('id', count='exact')
                .eq('is_active', True)
                .execute()
            )
        ...
        try:
            # 尝试从搜索日志表获取今日搜索次数
            today_searches_result = (
                supabase_service.client.table('search_logs')
                .select('id', count='exact')
        ...
        return APIResponse(success=True, message="获取系统统计成功", data={"stats": stats})

    except Exception as e:
        logger.error(f"获取系统统计失败: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="获取系统统计失败")


@router.post("/users/{user_id}/subscription", response_model=APIResponse)
async def assign_user_subscription(user_id: str, body: dict, current_user: UserInfo = Depends(require_admin)):
    """
    为指定用户分配/切换套餐（管理员）。
    ...
    try:
        ...
        sub = await sub_service.assign_subscription(
            user_id,
            plan_slug,
...
async def cancel_user_subscription(user_id: str, body: dict = None, current_user: UserInfo = Depends(require_admin)):
    """
...
async def user_action(
    ...
    try:
        ...
        if success:
            return APIResponse(
                success=True, message=f"用户{action}操作成功", data={"user_id": user_id, "action": action}
            )
        else:
...
async def get_users_list(
    ...
    try:
        ...
        users_result = (
            supabase_service.client.table('profiles')
            .select('id, username, email, full_name, is_admin, user_level_name, is_active, created_at, last_login')
        ...
        return APIResponse(
            success=True,
            message="获取用户列表成功",
            data={"users": users, "total": total, "page": page, "limit": limit, "pages": (total + limit - 1) // limit},
        )
...
async def get_user_stats(
    ...
    try:
        ...
        return APIResponse(success=True, message="获取用户统计成功", data=stats)
    except Exception as e:
        logger.error(f"获取用户统计失败: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="获取用户统计失败")


@router.get("/monitor-settings", response_model=APIResponse)
async def get_monitor_settings(current_user: UserInfo = Depends(get_current_active_user)):
    """
    获取监控设置
    """
    try:
        ...
        return APIResponse(success=True, message="获取监控设置成功", data=settings)

    except Exception as e:
        logger.error(f"获取监控设置失败: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="获取监控设置失败")


@router.put("/monitor-settings", response_model=APIResponse)
async def update_monitor_settings(settings: dict[str, Any], current_user: UserInfo = Depends(get_current_active_user)):
    """
    更新监控设置
    """
    try:
        ...
        return APIResponse(success=True, message="监控设置更新成功", data={"settings": settings})

    except Exception as e:
        logger.error(f"更新监控设置失败: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="更新监控设置失败")


@router.get("/monitor-status", response_model=APIResponse)
async def get_monitor_status(current_user: UserInfo = Depends(get_current_active_user)):
    """
    获取监控系统状态
    """
    try:
        ...
        # 获取管理员用户数量
        admin_users_result = (
            supabase_service.client.table('profiles').select('id', count='exact').eq('is_admin', True).execute()
        )
        ...
        active_tasks_result = (
            supabase_service.client.table('monitor_tasks').select('id', count='exact').eq('is_active', True).execute()
        )
        active_tasks = active_tasks_result.count if active_tasks_result.count is not None else 0

        # 获取24小时内有检查记录的任务
        twenty_four_hours_ago = (datetime.now() - timedelta(hours=24)).isoformat()
        recent_active_tasks_result = (
            supabase_service.client.table('monitor_tasks')
            .select('id', count='exact')
            .gte('last_check', twenty_four_hours_ago)
            .execute()
        )
        ...
        status_data = {
            ...
            "tasks": {"total": total_tasks, "active": active_tasks, "recent_active": recent_active_tasks},
            "last_update": datetime.now().isoformat(),
        }

        return APIResponse(success=True, message="获取监控状态成功", data=status_data)

    except Exception as e:
        logger.error(f"获取监控状态失败: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="获取监控状态失败")
